...
echo "🎯 報告送信先: CEO Main (ペイン $CEO_PANE)"

# CEOに報告送信
tmux send-keys -t "$SESSION_NAME:$CEO_PANE" " && echo '【Director報告】$DIRECTOR_ID → CEO Main' && echo '⏰ 時刻: $TIMESTAMP' && echo '📋 部門報告: $REPORT_MESSAGE' && echo '✅ CEO報告確認。戦略判断に反映します。' ultrathink" Enter
