...
import { CloseHistory, CloseHistoryFilters, CloseHistoryStats } from "./types";
import { useCloseHistory } from "./useCloseHistory";
import { exportToCSV, exportToJSON, exportToExcel, checkExportSize } from "./exportUtils";
...
export function CloseHistoryTable({ refreshTrigger, accounts }: CloseHistoryTableProps) {
  ...
  const handleExport = (format: 'csv' | 'excel' | 'json') => {
    ...
    const exportCheck = checkExportSize(data.items);
    ...
    const filename = `close_history_${timestamp}`;

    try {
      switch (format) {
        case 'csv':
          exportToCSV(data.items, data.stats, `${filename}.csv`);
          break;
        case 'excel':
          exportToExcel(data.items, data.stats, `${filename}.xls`);
          break;
        case 'json':
          exportToJSON(data.items, data.stats, `${filename}.json`);
          break;
      }
    } catch (error) {
