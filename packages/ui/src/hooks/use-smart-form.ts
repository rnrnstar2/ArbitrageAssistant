...
interface SmartFormOptions<TFormData extends FieldValues> {
  ...
  // 成功時の処理
  onSuccess?: (data: TFormData, result?: unknown) => void;
 ...
 * ```
 */
export function useSmartForm<TFormData extends FieldValues = FieldValues>(
    ...
    (submitFn: (data: TFormData) => Promise<unknown>) =>
      async (data: TFormData) => {
        ...
        try {
          ...
          // エラー処理
          const errorMessage = (err as Error)?.message || '処理中にエラーが発生しました';
          ...
          // デバッグ情報
          console.error('Smart Form Error:', err instanceof Error ? err.message : String(err), {
            data,
